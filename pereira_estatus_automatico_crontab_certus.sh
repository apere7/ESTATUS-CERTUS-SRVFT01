...
RUTINA_PING () {
       ...
       if [ "$valor1" = "0%" ]; then
        ...
        elif [ "$valor1" = "100%" ]; then
           echo "Tiempo de Respuesta Servidor " $servidor "NOT OK"
        else
          # 6 packets transmitted, 6 packets received, 0% packet loss
          # 6 packets transmitted, 5 packets received, 16% packet loss 
          echo "Tiempo de Respuesta Servidor es lento " $servidor "Revisar" 
       fi
...
mailx -s "CERTUS - Estatus Diario $DIA "  apere7@cantv.com.ve  < salida_estatus_crontab_certus.txt
